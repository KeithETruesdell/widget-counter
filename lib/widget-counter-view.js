...
export default class WidgetCounterView {
	...
	updateCounterView(calledOnLoad) {
		this.setConfigValues();
	...
	postToSlack() {
		if (this.slackChannel != "" && this.slackToken != "") {
			...
			var sendText = encodeURI(slackText + " " + this.widgetValue + " " + this.widgetName + (this.widgetValue > 1 && (this.widgetName).slice(-1).toLowerCase() != "s" ? "s" : "") + ((this.widgetName).slice(0, 5).toLowerCase() != "count" ? " counted" : ""));
